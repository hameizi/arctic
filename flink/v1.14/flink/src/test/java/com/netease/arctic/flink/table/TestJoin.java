...
//    sql("create table d (op_time timestamp(3), watermark for op_time as op_time) like %s", table);
  ...
  @Test(timeout = 180000)
  public void testLookupJoinNewProject() throws Exception {
...
//    sql("create table d (op_time timestamp(3), watermark for op_time as op_time) like %s", table);
    ...
    TableResult result = exec(
        String.format(
            "select u.name from ( select *, proctime() as ptm from %s " +
                "/*+OPTIONS('arctic.read.mode'='log','log-store.enabled'='true','log-store.address'='1.1.1.1','log-store.topic'='sss','scan.startup.mode'='latest','properties.group.id'='fengxin-test-flink-read-arctic')*/ ) as u " +
                "left join  %s /*+OPTIONS('lookup.cache.max-rows'='10002')*/ for system_time as of u.ptm as dim on u.id = dim.id  ",
            table,
            table
        ));
